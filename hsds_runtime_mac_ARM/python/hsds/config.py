...
def _load_cfg():
    ...
    if getattr(sys, 'frozen', False):
        config_dir = os.path.join(os.path.dirname(sys.executable), 'hsds_runtime')
        ...
        if not config_dirs:
            ...
            if getattr(sys, 'frozen', False):
                config_dirs.append(os.path.join(os.path.dirname(sys.executable), 'hsds_runtime'))
        for config_dir in config_dirs:
            ...
            if os.path.isfile(file_name):
                yml_file = file_name
                break
        yml_file = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'config.yml')
        if not yml_file:
