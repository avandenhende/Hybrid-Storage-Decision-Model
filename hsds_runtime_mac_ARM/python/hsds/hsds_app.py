...
def get_cmd_dir():
    if getattr(sys, 'frozen', False):
        return os.path.join(os.path.dirname(sys.executable), 'hsds_runtime')
    else:
...
class HsdsApp:
    ...
    def run(self):
        ...
        #py_exe = os.path.join('/usr', 'bin', 'python3')
        py_exe = os.path.join(self._cmd_dir, 'python3', 'bin', 'python3')
        ...
        for i in range(count):
            ...
            p = subprocess.Popen(
                ...
                stderr=subprocess.STDOUT, encoding='utf-8', env=env
            )
